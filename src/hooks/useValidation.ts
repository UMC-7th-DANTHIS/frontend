...
const useValidation = (formState: FormState, type: IS_FOR): boolean => {
  const [isValid, setIsValid] = useState<boolean>(false);

  useEffect(() => {
    let validationRules: Record<string, boolean> = {};

    if (type === IS_FOR.DANCER && 'dancerName' in formState) {
      validationRules = {
        isDancerNameValid: formState.dancerName.trim().length > 0 && formState.dancerName.trim().length <= 20,
        isInstagramIdValid: formState.instargramId.trim().length > 0 && formState.instargramId.trim().length <= 20,
        isOpenChatUrlValid: formState.openChatUrl.trim().length > 0 && formState.openChatUrl.trim().length <= 255,
    ...
    } else if (type === IS_FOR.CLASS && 'className' in formState) {
      validationRules = {
        ...
        isHashtagsValid: formState.hashtags.length > 0 && formState.hashtags.length <= 3
      };
    }
